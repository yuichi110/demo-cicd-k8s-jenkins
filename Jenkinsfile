  ...
  stages {
    ...
    stage('CD:Deploy') {
      steps {
        ...
        sh 'kubectl --kubeconfig kubeconfig apply -f ./k8s/db/deployment.yml'
        sh 'kubectl --kubeconfig kubeconfig apply -f ./k8s/db/service.yml'
        sh "sed -e 's/{{DOCKERHUB_USER}}/${DOCKERHUB_USER}/g' -e 's/{{BUILD_TIMESTAMP}}/${BUILD_TIMESTAMP}/g' ./k8s/back/deployment.yml | kubectl --kubeconfig kubeconfig apply -f -"
        sh 'kubectl --kubeconfig kubeconfig apply -f ./k8s/back/service.yml'
        sh "sed -e 's/{{DOCKERHUB_USER}}/${DOCKERHUB_USER}/g' -e 's/{{BUILD_TIMESTAMP}}/${BUILD_TIMESTAMP}/g' ./k8s/front/deployment.yml | kubectl --kubeconfig kubeconfig apply -f -"
        sh 'kubectl --kubeconfig kubeconfig apply -f ./k8s/front/service.yml'
      }
    }
  }
}